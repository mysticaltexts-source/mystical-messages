        ...
        <div className="fade-up-1" style={{ background:"rgba(255,255,255,0.04)", backdropFilter:"blur(16px)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:20, padding:32 }}>
          ...
          {mode === "signup" && (
            <p style={{ fontSize:12, color:"rgba(255,255,255,0.3)", textAlign:"center", marginTop:18, lineHeight:1.7 }}>
              By signing up you confirm you are 18+ and agree to our Terms of Service. Adults-only platform — children never have accounts.
            </p>
          )}
        </div>
...
/* ══════════════════════════════════════
   SCREEN: SETUP (first login — capture phone number)
══════════════════════════════════════ */
...
function DashboardScreen({ session, profile, onGoToBilling, onGoToHistory, onGoToSchedule, onGoToProfiles, onLogout }) {
  ...
  async function sendOhCrap(btn) {
    ...
    const body = getOhCrapTemplate(btn.charSlug, childName);
  ...
  return (
    <div style={{ minHeight:"100vh", background:T.parchment }}>
      ...
      <div style={{ flex:1, maxWidth:860, margin:"0 auto", width:"100%", padding:"32px 24px 60px" }}>
    ...
    <div style={{ minHeight:"100vh", background:T.parchment }}>
      ...
      <div style={{ maxWidth:900, margin:"0 auto", padding:"40px 24px 80px" }}>
        <div className="fade-up" style={{ marginBottom:40 }}>
          ...
          {currentPlan !== "free" && (
            <div style={{ marginTop:20, background:T.warmWhite, border:`1.5px solid ${T.gold}`, borderRadius:14, padding:"18px 22px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
              <div style={{ display:"flex", alignItems:"center", gap:14 }}>
                <span style={{ fontSize:28 }}>🌠</span>
                <div>
                  <div style={{ fontSize:15, fontWeight:600, color:T.ink }}>{currentPlan.charAt(0).toUpperCase()+currentPlan.slice(1)} Plan</div>
                  <div style={{ fontSize:13, color:T.muted }}>Managed via Stripe · Cancel anytime</div>
                </div>
              </div>
              <div style={{ display:"flex", gap:10 }}>
                <span style={{ background:"rgba(74,140,110,0.1)", color:T.success, border:"1px solid rgba(74,140,110,0.25)", fontSize:12, fontWeight:600, padding:"4px 12px", borderRadius:100 }}>✓ Active</span>
                <button onClick={() => setCancelConfirm(true)} style={{ background:"none", border:"none", color:T.muted, fontSize:13, cursor:"pointer", textDecoration:"underline" }}>Cancel</button>
              </div>
            </div>
          )}
        </div>
...
function getOhCrapTemplate(slug, childName) {
  const t = {
    santa:        `Ho ho ho, ${childName}! 🎅 A little elf just sent me a message about what is happening at your house right now. Remember — I see everything, and the nice list is still open. Sweet dreams! ⭐`,
    tooth_fairy:  `Hello ${childName}! 🧚‍♀️ Word reached me that a very brave child lost a tooth tonight. I am already on my way — expect some sparkle under your pillow by morning. Sleep tight! 💫`,
    easter_bunny: `Hippity hoppity, ${childName}! 🐰 I heard you were looking for me. I am hopping as fast as I can — big things are coming your way very soon! 🥚🌸`,
  };
  return t[slug] || `A little magic is coming your way, ${childName}! ✨`;
}

/* ══════════════════════════════════════
   ROOT APP — real session management
══════════════════════════════════════ */
export default function App() {
  ...
  async function loadProfile(userId) {
    ...
    setProfile(data);
    // New user with no phone number goes to setup screen
    ...
    <>
      ...
      {screen === "dashboard" && session && (
        <DashboardScreen
          session={session}
          profile={profile}
          onGoToBilling={()   => setScreen("billing")}
